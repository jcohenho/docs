...
When invoking the [`delete()`](/concepts/components/prisma-client/crud#delete-a-single-record) or [`deleteAll()`](/concepts/components/prisma-client/crud#delete-all-records) methods using the Prisma Client on required relations, a runtime check is performed and the deletion of records prevented if they are referencing related objects. **This prevents cascade behavior, no matter how the foreign key is defined**.
...
If you are using either the [`delete()`](/concepts/components/prisma-client/crud#delete-a-single-record) or [`deleteAll()`](/concepts/components/prisma-client/crud#delete-all-records) methods, **cascading deletes will now be performed, as the safety net in Prisma Client that previously prevented cascading deletes at runtime is removed**. Be sure to check your code and make any adjustments accordingly.
