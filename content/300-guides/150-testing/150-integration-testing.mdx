...
At the time of writing, this guide uses `jest-mock-extended` version `^2.0.4`. See [mocking the Prisma Client](/guides/testing/unit-testing#mocking-the-prisma-client) for information on how to use this version.
...
The above `.env.test` file is used as part of a multiple `.env` file setup. Checkout the [using multiple .env files.](/guides/development-environment/environment-variables/using-multiple-env-files) section to learn more about setting up your project with multiple `.env` files
