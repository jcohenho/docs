...
When using the [`$transaction` API](/concepts/components/prisma-client/transactions#the-transaction-api), this behavior makes it possible for the client to pass all the queries on to the query engine as a single transaction.
